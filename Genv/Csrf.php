 ...
 * 
 */
class Genv_Csrf extends Genv_Base
{
    /**
     * 
    ...
    {
        if (! self::$_session) {
            self::$_session = Genv::factory('Genv_Session', array(
                'class' => 'Genv_Csrf',
            ));
        }
        ...
        $key = Genv_Config::get('Genv_Csrf', 'key');
        if ($key) {
            self::$_key = $key;
        }
    }
