 ...
 * PHP 5.2.0+, Genv_Json allows users to get a jump on JSON encoding and
 * decoding early if the native json_* functions are not present.
 * 
 * Genv_Json::encode and Genv_Json::decode functions are designed
 ...
 * @category Genv
 * 
 * @package Genv_Json JSON data formatting and checking.
 * 
 ...
 * 
 */
class Genv_Json extends Genv_Base
{
    /**
     * 
     * Default configuration values.
     * 
     * @config bool bypass_ext Flag to instruct Genv_Json to bypass
     *   native json extension, if installed.
     * 
     * @config bool bypass_mb Flag to instruct Genv_Json to bypass
     *   native mb_convert_encoding() function, if
     *   installed.
     * 
     * @config bool noerror Flag to instruct Genv_Json to return null
    ...
    public function decode($encodedValue, $asArray = false)
    {
        ...
        // Fall back to PHP-only method
        $this->_level = 0;
        $checker = Genv::factory('Genv_Json_Checker');
     ...
     * @return mixed decoded value
     * 
     * @todo Rewrite this based off of method used in Genv_Json_Checker
     * 
     */
